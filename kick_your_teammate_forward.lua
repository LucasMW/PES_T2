local function read_file(path_to_file, do_after)
...
local function filter_chars(str_data, do_after)
	do_after(str_data:gsub("[%W_]", " "), scan)
end

local function normalize(str_data, do_after)
	do_after(str_data:lower(), remove_stop_words)
end

local function scan(str_data, do_after)
	do_after(reverse_split(str_data, "%S+"), frequencies)
end

local function remove_stop_words(word_list, do_after)
	local stop_words_file = assert(io.open("input/stop_words.txt"), "erro na abertura de arquivo de stop_words")
	local stop_words = reverse_split(stop_words_file:read(), "[^,]+")
...
local function frequencies(word_list, do_after)
